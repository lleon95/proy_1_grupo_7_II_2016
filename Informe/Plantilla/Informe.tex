...
\textbf{Trabajo} & Control y programación RTC con & \textbf{Actualizado en:} & 06/06/2016 \\
\textbf{} &  picoblaze & \textbf{Revisado en:} & 07/04/2016 \\
\textbf{Grupo} & 1 & \textbf{Diseñadores} & Johan Arrieta Solórzano \\
\textbf{Revisado por:} & Alfonso Chacón Rodríguez & \textbf{ } & Melissa Fonseca Rodríguez 
\end{tabular}}
...
\section{Resumen} 
La implementación de un reloj de tiempo real se puede ver simplificada mediante el uso de un microcontrolador. Utilizando el picoblaze que nos proporciona la Nexys 3 se realiza un control para el picoblaze, siempre utilizando parte de la programación realizada anteriormente como driver.\\[2ex]
Para mejorar la interacción del usuario con el dispositivo se utiliza un teclado, para el cual se realiza el control respectivo; y para la interfaz visual se realiza una mejora utilizando imágenes en lugar de implementar toda la interfaz por medio de una ROM.\\[2ex]
En el presente se muestra la forma de implementación de todos los cambios mencionados y además de las partes que ya estaban presentes de proyectos anteriores; desde el diagrama modular hasta la implementación y los resultados obtenidos.

\section{Introducción} 
El PicoBlaze utilizado para este proyecto es el KCPSM6 el cual es el picoblaze optimizado para el uso en la Spartan-6, Virtex-6 y dispositivos de la serie 7. KCPSM6 define un microcontrolador de 8 bits el cual puede ser incluido una o más veces en cualquier diseño de Spartan-6... Probablemente unas de las mayores fortalezas es que está 100\% embebido y requiere solo 26 segmentos lógicos y un bloque de memoria que equivale al 4.3\% de la más pequeña XC6SLX4 y tan solo 0.11\% del XC6SLX150T. Esta combinación implica que se puede decidir cuándo y dónde poner el KCPSM6 mientras desarrolla su diseño en vez de requerir algún plan previo. \cite{DatasheetPico} \\[2ex]
La razón por la que se integra un pequeño procesador en el diseño de la FPGA se resumen en: el hardware es paralelo y los procesadores secuenciales. Por lo que convirtiendo una pequeña parte de hardware en un procesador es a menudo una forma más eficiente de implementar funciones secuenciales como máquinas de estados (especialmente las complejas) o recursos de harware a tiempo compartido cuando hay tareas bastante lentas a ser ejecutadas. También es más natural describir tareas secuenciales en software mientras que HDL es mejor describiendo harware paralelo. \cite{DatasheetPico}\\[2ex]
Para el RTC se necesita una máquina de estados que controle los procesos de tiempo que se deben realizar en el V3023 para lograr el funcionamiento del mismo, por lo cual ésta se puede implementar en el picoblaze, lo cual simplificará a gran manera la implementación.\\[2ex]
En cuanto a la interfaz, por lo experimentado en el proyecto pasado, realizar los cambios con los botones e interruptores que posee la Nexys es un poco grotesco. Y además la interfaz visual al realizarla toda por medio de una ROM y escalarla hace que ésta no sea tan amigable al usuario. Con estas consideraciones la implementación de la interfaz visual con imágenes y al utilizar un teclado se puede generar una interfaz más cómoda al usuario y con la cual se pueden generar cambios en la implementación del sistema general que pueden ayudar a mejorar y simplificar el diseño.
